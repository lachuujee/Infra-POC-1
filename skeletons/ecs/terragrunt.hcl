...
locals {
  ...
  name      = try(local.cfg.modules[local.component].name, "${local.base}_ecs")
...
inputs = {
  ...
  alb_state_key       = "wbd/sandbox/${local.intake_id}/lb/terraform.tfstate"
}
