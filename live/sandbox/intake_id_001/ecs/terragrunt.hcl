# Simple: ECS Terragrunt config

# VPC, LB first
dependencies {
  paths = ["${local.rel_up}/vpc", "${local.rel_up}/lb"]
}
...
locals {
  ...
  # Component
  component = basename(local.this_dir)  # "ecs"
  ...
  # Uniform Name: sbx_intake_id_001-ecs-dev
...
inputs = {
  ...
  alb_state_key       = "${local.state_prefix}/lb/terraform.tfstate"
}

# File: terragrunt.hcl (ecs)
